...
class RobotState(State):
    def __init__(self, board, parent=None, position=None, goal_position=None):
        ...
        # TODO 6: prosiriti stanje sa informacijom da li je robot pokupio oruzje
        self.oruzje = []
        row, col = self.position
        if self.parent is not None:
            self.oruzje = deepcopy(self.parent.oruzje)
            if self.board.data[row][col] == 'pl' and (row, col) not in self.oruzje:
                self.oruzje.append((row, col))
        self.oruzje_positions = board.find_position('pl')
    ...
    def get_legal_positions(self):
        ...
        row, col = self.position  # trenutna pozicija
        new_positions = []

        if len(self.oruzje) < 2:
            d_rows = [1, 1, -1, -1]
            d_cols = [-1, 1, -1, 1]
            ...
            for d_row, d_col in zip(d_rows, d_cols):  # za sve moguce smerove
                ...
                while 0 <= new_row < self.board.rows and 0 <= new_col < self.board.cols and self.board.data[new_row][
                    new_col] != 'w':
    ...
    def is_final_state(self):
        if self.oruzje_positions[0] is not None:
            return self.position == self.goal_position and len(self.oruzje) == 2

    def unique_hash(self):
        return str(self.position) + str(self.oruzje)
